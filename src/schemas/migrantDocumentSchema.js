...
const migrantDocumentSchema = Joi.object({
    document_type: Joi.string().min(3).max(120).required().messages(messages.document_type),
    document_identification: Joi.string().min(3).max(120).required().messages(messages.document_identification),
});
