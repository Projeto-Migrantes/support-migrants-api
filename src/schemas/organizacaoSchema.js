...
const organizacaoEsquema = Joi.object({
    ...
    telefonePrincipal: Joi.string().pattern(/^\(\d{2}\) \d{5}-\d{4}$/).required().messages(mensagens.telefonePrincipal),

    // Validando Telefone opcional
    telefoneSecundario: Joi.string().pattern(/^\(\d{2}\) \d{5}-\d{4}$/).optional().messages(mensagens.telefoneSecundario),
