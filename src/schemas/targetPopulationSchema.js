...
const targetPopulationSchema = Joi.object({
    target_populations_pt: Joi.string().min(3).max(500).required().messages(messages.target_population_pt),
    target_populations_en: Joi.string().min(3).max(500).required().messages(messages.target_population_en),
    target_populations_es: Joi.string().min(3).max(500).required().messages(messages.target_population_es),
    target_populations_fr: Joi.string().min(3).max(500).required().messages(messages.target_population_fr),
});
